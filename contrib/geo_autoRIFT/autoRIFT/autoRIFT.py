...
#                   Dx1[ii], Dy1[ii] = arPixDisp(ChipI,RefI)
        ...
        I_shape = I1.shape
        shape_prod = int(np.prod(in_shape))
        
        #        import pdb
        #        pdb.set_trace()
        XI1 = mp.RawArray('b', int(np.prod(I_shape)))
        XI1_np = np.frombuffer(XI1,dtype=np.uint8).reshape(I_shape)
        np.copyto(XI1_np,I1)
        del I1
        
        XI2 = mp.RawArray('b', int(np.prod(I_shape)))
...
#                   Dx1[ii], Dy1[ii] = arPixDisp(ChipI,RefI)
        ...
        #        import pdb
        #        pdb.set_trace()
        XI1 = mp.RawArray('f', int(np.prod(I_shape)))
        XI1_np = np.frombuffer(XI1,dtype=np.float32).reshape(I_shape)
        np.copyto(XI1_np,I1)
        del I1
        
        XI2 = mp.RawArray('f', int(np.prod(I_shape)))
